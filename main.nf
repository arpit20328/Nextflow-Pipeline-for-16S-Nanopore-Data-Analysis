...
process Subsample {
        ...
        script:
        """
        seqkit sample -n 125000 -j 64 ${params.sequences}/${Sample}.fastq.gz > ${Sample}_subsampled.fastq
        """
}
...
process Coverage {
    tag "$Sample"
    cpus 150
    publishDir "${params.outdir ?: PWD}/Final_Output/${Sample}/${Sample}_coverage_summary", mode: 'copy'
    ...
    bash /goast/arpit_data/nextflow_rifaximin_pipleine/scripts/coverage.sh \
        ${filtered_fastq} ${Sample}_coverage_summary.tsv $task.cpus
...
process NanoPlot {
        ...
        script:
        """
        NanoPlot -t 120 --fastq ${filtered_fastq} --N50 -f png -o ${Sample}_NanoPlot_Report
        """
}
...
process EMU {
        ...
        script:
        """
        emu abundance --type map-ont ${filtered_fastq} --db /goast/arpit_data/nextflow_rifaximin_pipleine/emu_database/ --threads 170 --min-abundance 0.0001  --output-dir ${Sample}_emu_results
...
workflow NANOPORE_16S {
    ...
    // ✅ Add Coverage.out to the join so Patient_report gets coverage files
    Patient_report(
        INDEX_CALCULATION.out
            .join(BARPLOT_TABLE_EMU.out)
            .join(NanoPlot.out)
            .join(Coverage.out)
    )
}
